...
|---|---|
| Window shell | The Win32 host window + the manual 30 Hz think-loop; a menu bar with a persistent light/dark theme toggle (seeded before the page is first shown, so a saved dark theme never flashes light); and three footer groups for the connection dot/version plus selection, the live read-only camera X/Y/Z origin, and Updated/help. The green dot is the sole visible connection indicator. Content sections share an adjustable, keyboard-accessible vertical splitter framework (including both Assets dividers), with one 40px header track on either side. At compact widths the camera group reflows onto a second row, while the page remains clamped to the WebView client area so header and footer never scroll out of or get covered within the viewport, including in Decl Text focus mode. Camera editing and locking are intentionally absent from the current UI, while their native bridge remains available for a future control. |
