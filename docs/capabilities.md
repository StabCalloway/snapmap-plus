...
| Dynamic decl server | Cold-start registration for genuinely new text decl identities under `%LOCALAPPDATA%\snapmap-plus\overrides\generated\decls\<type>\...*.decl`. The first directory is the engine decl type; the remaining path without `.decl` is the logical name. Existing identities remain ordinary file shadows (`SHADOWED`); absent identities are inserted through DOOM's native dynamic-decl API (`REGISTERED`); unsafe, ambiguous, unsupported, or unpublished candidates are `REFUSED`. One immutable process snapshot, no refresh/hot reload/unload. This does not supply models, sounds, images, or other binary dependencies. |
