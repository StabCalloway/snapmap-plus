 ...
 *
 * MECHANISM (DIRECT, our own reverse-engineering, 2026-07-27).
 ...
 *
 * PORTABILITY: the handler resolves by SIGNATURE (never a hardcoded RVA -- our own reverse-engineering proved
 ...
 * (whole field when nothing is selected) to the Windows clipboard, and Ctrl+V splices the clipboard
 * back in at the selection. No-op + logged reason if the signature does not resolve cleanly. Paste
 * additionally needs its own idStr-assignment resolve: if that misses, the detour still arms
 * COPY-ONLY rather than guessing at a write. Safe to call once at backend install. */
